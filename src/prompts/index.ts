      ...
      messages: [{
        ...
        content: {
          ...
          text:
            ...
            `1. Use the \`research_topic\` tool to get a multi-source overview (costs $0.02)\n` +
            `2. Review the sources returned. For the 2-3 most relevant ones, use \`fetch_url\` to get the full content ($0.001 each)\n` +
            `3. If any source is a supported platform (GitHub, LinkedIn, Crunchbase, etc.), use \`extract_data\` instead for structured information ($0.008)\n` +
            ...
            `Estimated cost: $0.023-$0.045 depending on depth.`,
        },
      }],
    }),
  );
      ...
      messages: [{
        ...
        content: {
          ...
          text:
            ...
            `Workflow:\n` +
            `1. Use \`research_topic\` for "${subject} vs ${competitors}" to get market context ($0.02)\n` +
            `2. For each company/product with a website:\n` +
            `   - Use \`fetch_url\` on their homepage/product page ($0.001 each)\n` +
            `   - If they have a GitHub repo, use \`extract_data\` for repo metrics ($0.008)\n` +
            `   - If they have a Crunchbase/LinkedIn page, use \`extract_data\` for company data ($0.008)\n` +
            ...
            `Estimated cost: $0.03-$0.09 depending on number of competitors.`,
        },
      }],
    }),
  );
        ...
        messages: [{
          ...
          content: {
            ...
            text:
              ...
              `Workflow:\n` +
              `1. Use \`suggest_domains\` with the concept to get AI-generated suggestions ($0.003)\n` +
              `2. Review results. For any promising names that were not checked, use \`check_domains\` ($0.003)\n` +
              ...
              `Estimated cost: $0.003-$0.015 depending on how many batches you check.`,
          },
        }],
      };
    },
      ...
      messages: [{
        ...
        content: {
          ...
          text:
            ...
            `2. Use \`render_page\` to see the JS-rendered version — compare with fetch to identify dynamic content ($0.005)\n` +
            `3. If the site is on a supported platform (GitHub, Product Hunt, etc.), also use \`extract_data\` ($0.008)\n` +
